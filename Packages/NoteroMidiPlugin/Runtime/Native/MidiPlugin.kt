...
class NoteroMidiInputPort(private val inputPort: MidiInputPort, private val midiDevice: MidiDevice) {
...
abstract class NoteroMidiDevice(private val target: MidiDeviceInfo, private val inputPort: NoteroMidiInputPort) {
...
class MidiPlugin(private val _context: Context) {
    ...
    fun autoManageDevice() {
        ...
        val midiManager: MidiManager = _context.getSystemService(Context.MIDI_SERVICE) as MidiManager
    ...
    private fun sendVolumeControlMessage(volumeAsPercent: Int, devices: List<NoteroMidiDevice> = out) {
    ...
    fun discoverAllMidiInputPort() {
        ...
        for (device in devices) {
            val deviceName: String = device.properties.getString(MidiDeviceInfo.PROPERTY_NAME) ?: continue
    ...
    fun discoverMidiInputPort(target: MidiDeviceInfo) {
        ...
        val midiManager: MidiManager = _context.getSystemService(Context.MIDI_SERVICE) as MidiManager
        val inputPortInfo: MidiDeviceInfo.PortInfo? = target.ports.find { it.type == MidiDeviceInfo.PortInfo.TYPE_INPUT }
        ...
        midiManager.openDevice(target, {
            try {
                ...
                if (inputPort != null) {
                    ...
                    val noteroInputPort = NoteroMidiInputPort(inputPort, it)
    ...
    fun discoverMidiInputPort(deviceName: String) {
        ...
        val target: MidiDeviceInfo? = devices.find { it.properties.getString(MidiDeviceInfo.PROPERTY_NAME) == deviceName || it.properties.getString(MidiDeviceInfo.PROPERTY_PRODUCT) == deviceName }
        if (target == null) {
    ...
    fun closeMidiInputPort(deviceName: String) {
        Log.d(TAG, "get MIDI Device")
        val midiManager: MidiManager = _context.getSystemService(Context.MIDI_SERVICE) as MidiManager
        val devices = midiManager.devices.filter { it.inputPortCount > 0 }
    ...
    fun closeDevice(target: MidiDeviceInfo) {
        Log.d(TAG, "before close ported = ${target.id} [${target.properties.getString("name")}]")
        val closeDevice = out.find { it.getId() == target.id } ?: return

        closeDevice.close()
        out.remove(closeDevice)
        Log.d(TAG, "close ported = ${target.id} [${target.properties.getString("name")}]")
    }
