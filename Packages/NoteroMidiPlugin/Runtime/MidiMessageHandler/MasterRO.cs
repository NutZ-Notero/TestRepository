...
{
    public class MasterROMidiDeviceAdapter : MidiDeviceInfo
    {
        private MasterROMidiDeviceAdapter(int uniqueId, string deviceName) : base(uniqueId, deviceName)
        {
        ...
        {
            return deviceName.ToLower().StartsWith("master ro 61") || deviceName.ToLower().StartsWith("pop piano lite")
                ? new MasterROMidiDeviceAdapter(uniqueId, deviceName)
                : null;
        }
        ...
        public override byte[] GetVolumeControlMessage(int volumeAsPercent)
        {
            ...
            var stateByte = volumeAsPercent != 0 ? 0x77 : 0x78;
