...
{
    public class GZUTPianoMidiDeviceAdapter : MidiDevice
    {
        private MidiOut m_MidiOut;

        private GZUTPianoMidiDeviceAdapter(MidiOutCapabilities midiOutCapabilities, MidiOut midiOut) : base(midiOutCapabilities, midiOut)
        {
        ...
        private static MidiDevice Assert(MidiOutCapabilities midiDeviceInfo, MidiOut midiOut)
        {
            ...
            return deviceName.ToLower().StartsWith("gzu-tek") ? new GZUTPianoMidiDeviceAdapter(midiDeviceInfo, midiOut) : null;
        }
