...
{
    public class GeneralMidiDeviceAdapter : MidiDevice
    {
        private MidiOut m_MidiOut;

        private GeneralMidiDeviceAdapter(MidiOutCapabilities midiOutCapabilities, MidiOut midiOut) : base(midiOutCapabilities, midiOut)
        {
        ...
        private static MidiDevice Assert(MidiOutCapabilities midiDeviceInfo, MidiOut midiOut)
        {
            ...
            return deviceName.ToLower().StartsWith("general") ? new GeneralMidiDeviceAdapter(midiDeviceInfo, midiOut) : null;
        }
